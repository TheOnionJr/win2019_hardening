...
The module changes several settings on the machine, to mitigate several security-holes, as well as setting up auditing to Microsoft's "Strong Recommendation". (Which is recommended for systems where security and useability is important)


### What win2019_hardening affects 

 * Disables the SMB port, to prevent eternal-blue exploitation.
 * Changes/adds several registry entries, to mitigate several security-holes. (All changes can be seen in manifests/config.pp)
...
Clone this repository into C:\ProgramData\PuppetLabs\code\environments\production\modules (ProgramData is a hidden folder, if you are using GUI you might need to enable "Show hidden files, folders, and drives").
Install all the dependent puppet modules. (Links with instructions found under the Setup Requirements section further down)
...
The options are: "true" (it can _not_ reboot with users logged in) is default, change to "false" to allow reboots while a user is logged on. 
...
Limited to Windows. 
Other versions than Server2019 is unknown.
